...
### Additional Files/Logs
  * Additional files that log the processes used or capture information not described in this document may be delivered after consulting with PAMI staff. These files should be stored in the `tag` directory of the bag.
  * Naming convention: division_PrimaryID_v01_pm.mkv.tag_file_identifier
  * Directory location
      * PrimaryID
          * tags
          ...
          * data
              * PreservationMasters
              * ...
  * Checksums for files in the `tag` directory should be included in the tagmanifest-md5.txt file, but they are not required.  

<a name="qctools-reports"></a>
### QCTools Reports
  * Each video preservation master file should receive a corresponding QCTools report, which will be included in the optional `tag` directory of the bag (See additional files specifications).
