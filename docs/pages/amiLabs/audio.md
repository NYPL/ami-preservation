...
* There is one other configuration that you will come across if the tapes are of a more recent vintage:
  * **Two track stereo one direction**: Testing for this configuration is done the same way as **Two track double mono one direction**, except there will be a stereo signal from the two channels. When you switch from the 2 track head to the 4 track head, the loss of volume in the right channel will be approximately 6 dB.
...
* There are a number of references available to guide to engineer through this part of the restoration. On the Web, this [site](http://wiki.audacityteam.org/wiki/78rpm_playback_curves) contains useful information regarding the playback curves of pre-RIAA discs.
