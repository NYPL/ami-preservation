...
for file in *.mkv ; do
  ffmpeg -i "$file" -map 0:v -map 0:a -c:v libx264 -movflags faststart -pix_fmt yuv420p -b:v 3500000 -bufsize 1750000 -maxrate 3500000 -vf yadif -c:a aac -strict -2 -b:a 192000 -ar 48000 -y -s 720x486 -aspect 4:3 "${file%.*}_sc.mp4" ;
done
